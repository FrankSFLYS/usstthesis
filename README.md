...
1. 本项目中最重要的文件就是 `usstthesis.cls`，`cls` 文件即“Class”，文档类，如果你要从零开始创建自己的毕业论文项目文件夹，你可以将此文件复制到你的目录中，然后在 LaTeX 主文件（本项目的主文件是 `demo.tex`）中使用 
...
5. `demo.pdf` 是编译生成的文件，项目中保留此文件是为了方便查看效果；
...
此时注意在左上角的 `Menu（菜单）-> Compiler（编译器）` 中，选择 `XeLaTeX`，并且注意 `Main Document（主目录）` 选择 `demo.tex`，之后点击界面中的 `Recompile（重新编译）`，如果不出意外的话就可以得到 PDF 文件了。
