...
[download link]: https://www.lanzous.com/b689722/ "usstthesis-2.99792-含字体包.zip"

## 注意事项

学校并未规定使用（或不使用）LaTeX 进行论文写作，并且本文档类中部分格式暂无法做到与学校给出的 Word 版本完全相同，所以使用本模板类造成（包括但不限于）论文审核不通过等问题作者不负责，请使用时慎酌（**定心丸**：作者已经使用此模板并且通过了毕业论文答辩，而且没有老师看出有任何格式方面的瑕疵）。
...
1. 本项目中最重要的文件就是 `usstthesis.cls`，`cls` 文件即“Class”，文档类，如果你要从零开始创建自己的毕业论文项目文件夹，你可以将此文件复制到你的目录中，然后在 LaTeX 主文件（本项目的主文件是 `main.tex`）中使用 
...
2. 显然，所有的 `ttf` 字体文件一定要在根目录下，并且需要与 `usstthesis.cls` 中声明的文件名相同，否则编译不通过，由于包含了字体的项目压缩包下载较慢，你可以到[蓝奏云][download link]下载，密码 `24qa`；
...
6. `c.bat`、`c.sh`、`Generate.bat`、`Generate.sh` 是可执行的脚本，具体功能下面有介绍，你可以使用编辑器打开查看里面的内容；
...
3. 如果命令行窗口提示“xelatex 不是可用的命令”等，说明你没有正确配置环境变量，你需要将 LaTeX 的 bin 目录添加到环境变量的 path 中；如果 xelatex 正确运行但是报错了，则可以根据报错的内容查看错误来源，如果提示
   ...
   说明缺失字体，你可以[下载字体包][download link]，密码 `24qa`，将缺失的字体解压到项目文件夹中，再次编译即可，文档类会自动搜索字体文件。此处未提到的问题你也可以提交 issue。
...
3. 如果提示 `xelatex` 的命令不存在，可能是没有权限，也可能是没有正确设置 LaTeX 的环境变量，你可以搜索或查找文档进行解决，如果提示
   ...
   可能是因为没有相关字体，你可以[下载字体包][download link]，密码 `24qa`，将缺失的字体解压到项目文件夹中，再次编译即可，文档类会自动搜索字体文件。此处未提到的问题你也可以提交 issue。
...
5. 在 `MyThesis` 目录中终端输入 `./c.sh` 可以一步编译生成 `MyThesis.pdf`，并且在最后显示出文档的字数统计，字数仅包括正文字数（即不包括承诺书、摘要、ABSTRACT、引用、致谢等的字数），格式为
...
### 下方内容适用于无法找到模板的情况
首先下载[含字体包的项目压缩文件][download link]，密码 `24qa`，之后在 Overleaf 网页中点击 `New Project`（创建新项目），选择 `Upload Project`（上传项目），在弹出的窗口中拖入刚才下载的压缩文件，等待上传并处理完成后就会自动跳转到编辑界面。
...
3. 因为调整间距时产生了未知问题，最终的 section 无法与模板完全一致，但是差别较小可以忽略；
4. 使用 `subfigure` 时可能会出现无法编译的情况，请根据注释在 `usstthesis.cls` 中将 `RequirePackage{subfigure}` 相关行注释掉再编译；
5. 未进行极限情况的测试，例如目录超过 2 页、标题字数太多等情况，如果你在使用时出现了问题，可以提交 issue，我会在之后进行更新修复。
